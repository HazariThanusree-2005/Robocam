...
export default function MotionPresetsPage() {
  const { setPage, setCurrentPreset } = useApp();
  ...
  return (
    <div className="min-h-screen bg-transparent px-5 pt-12 pb-10 animate-fade-in">
      ...
      <PageHeader title="Motion Presets" subtitle="Saved cinematic motion paths" />
      ...
      {presets.length === 0 ? (
        ...
        <div className="space-y-4">
          {presets.map((preset) => {
            ...
            return (
              <div key={preset.id} className="glass rounded-3xl border border-white/8 overflow-hidden animate-slide-up">
                <div className="p-4">
                      ...
                      {isEditing ? (
                        <div className="flex-1 flex gap-2">
                          <input
                            autoFocus
                            type="text"
                            className="flex-1 bg-white/5 border border-white/20 rounded-xl px-3 py-1.5 text-sm outline-none focus:border-accent text-white"
                            value={editName}
                            onChange={e => setEditName(e.target.value)}
                            onKeyDown={e => { if (e.key === 'Enter') handleRenameSave(preset.id); }}
                          />
                          <button onClick={() => handleRenameSave(preset.id)} className="px-3 py-1.5 rounded-xl bg-accent/20 text-accent text-sm font-semibold hover:bg-accent/30 transition-colors">Save</button>
                        ...
                        <div className="flex-1 min-w-0 flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="text-white/40 hover:text-accent p-1 transition-colors"
                            aria-label="Rename"
                          >
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button
                      ...
                      className="py-2.5 rounded-2xl glass-blue text-accent text-sm font-semibold hover:brightness-110 transition-all duration-200 active:scale-95"
                    >
                      Open Preset
                    </button>
                  </div>
